...
\usepackage{xltxtra} % 提供\XeLaTeX命令，生成XeLaTeX的logo
%--tikz related--->
\usepackage{tikz}
...
\setcitestyle{super} %如果想在正文中的引用为上标形式，用super。想要引用的方括号和正文一样大小，注释此行。
...
%<-<-<-<-<-<-<-<-<-<-<-<-<-<-<-<-<-<-<-<-<-<-
\begin{document}
% 双语目录参考这个方案：http://blog.csdn.net/solstice/article/details/1589348
% 以下一段是生成英文目录的关键
\makeatletter
...
需要注意的是，把cumtunsrtnat.bst文件复制到\LaTeX{}的安装目录下面后，需要手动运行texhash命令刷新一下，否则bibtex找不到你自己的.bst文件。
\par
...
带一个终端模拟器git-bash，还提供了sed、grep、awk、find等强大的unix命令，连perl都有(虽然我并不会perl...)，非常好用。编辑器采用vim(也是windows版的git提供的)，notepad++也比较好，根据自己喜好选择自己的编辑器吧。像毕业论文这样的大工程，还是有一个版本控制工具比较好，随时查看历史版本，我用的是git，大家也可以用自己喜欢的。自从有了git，后悔药不再是梦想。
\par
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.8\textwidth]{pictures/gaussianAndDog.PNG}
...
总之，本模板是比较简陋的，但是，仍然希望本模板可以帮助想用\LaTeX{}写矿大本科毕业论文的同学节约一些时间，提供一些启发。
如果学校的格式要求变了，希望你在看了本模板后，自己在做自己的模板的时候能从中得到一些灵感。毕竟，决定要用\LaTeX{}写毕业论文的你，还是有两把刷子的。
\newpage
\phantomsection
